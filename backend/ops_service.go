...
func (s *OpsService) Search(ctx context.Context, q OpsQuery) (OpsPage, error) {
	...
	return OpsPage{Items: filtered[start:end], Page: q.Page, PageSize: q.PageSize, Total: len(items), HasNext: end < len(filtered)}, nil
}
...
// Rules returns the full rule set of the operations domain.
func (s *OpsService) Rules() []OpsRule { return opsRules() }
